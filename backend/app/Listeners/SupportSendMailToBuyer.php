        ...
        if (!empty($ticket_info->job_request_id)){
            if ($ticket_info->notify === 'on' && $ticket_info->type === 'seller'){
                ...
                if ($ticket_info->type === 'buyer'){
                    ...
                    $buyer_info = User::select('id', 'email')->find($job_request_details->buyer_id);
                ...
                $message .= '<div><a href="'.route('buyer.job.request.conversation',$ticket_info->job_request_id).'">'.__('check messages').'</a></div>';
                $message .= '</p>';
