...
class ServiceController extends Controller
{
    ...
    {
        $this->middleware('permission:service-list|service-status|service-delete|service-view', ['only' => ['index', 'apiServices']]);
        $this->middleware('permission:service-status', ['only' => ['change_status', 'apiUpdateService', 'apiCreateService', 'apiUpdateServiceStatus', 'apiBulkStatus']]);
        $this->middleware('permission:service-delete', ['only' => ['delete_service', 'bulk_action', 'apiDeleteService', 'apiBulkDelete']]);
    ...
    {
        $services = Service::with(['seller', 'category', 'subcategory', 'childcategory'])
            ->orderByDesc('id')
            ->get()
    ...
    public function apiCreateService(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'service' => $this->formatServicePayload($service->fresh(['seller', 'category', 'subcategory', 'childcategory'])),
        ]);
    }

    public function apiUpdateService(Request $request, $id): JsonResponse
    {
        $validated = $request->validate([
            'title_en' => 'required|string|max:191',
            'title_ar' => 'nullable|string|max:191',
    ...
    public function apiUpdateServiceStatus(Request $request, $id): JsonResponse
    {
        $validated = $request->validate([
        ...
        return response()->json([
            'status' => 'success',
            'message' => __('Service status updated successfully'),
            'service' => $this->formatServicePayload($service->fresh(['seller', 'category', 'subcategory', 'childcategory'])),
        ]);
    }

    public function apiDeleteService($id): JsonResponse
    {
    ...
    public function apiBulkStatus(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'ids' => 'required|array',
            'ids.*' => 'integer',
        ...
        return response()->json([
            'status' => 'success',
            'message' => __('Services updated successfully'),
        ]);
    }
    ...
    public function apiBulkDelete(Request $request): JsonResponse
    {
        ...
        Serviceinclude::whereIn('service_id', $validated['ids'])->delete();
        Serviceadditional::whereIn('service_id', $validated['ids'])->delete();
        Servicebenifit::whereIn('service_id', $validated['ids'])->delete();
        OnlineServiceFaq::whereIn('service_id', $validated['ids'])->delete();
