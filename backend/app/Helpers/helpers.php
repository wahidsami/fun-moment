...
function get_default_language()
{
    ...
    return $defaultLang->slug ?? config('app.locale', 'en');
}
...
function get_user_lang_direction()
{
    ...
    return !empty(session()->get('lang'))
        ? ($user_direction?->direction ?? $default?->direction ?? 'ltr')
...
function admin_default_lang(){
    ...
    return $default_lang->slug ?? config('app.locale', 'en');
}
function front_default_lang(){
    $default_lang = !empty(session()->get('lang'))
        ? session()->get('lang')
        : (Language::where('default', 1)->first()->slug ?? config('app.locale', 'en'));
    return $default_lang;
}
...
function moduleExists($name){
    $status_file = __DIR__.'/../../modules_statuses.json';
    if (!file_exists($status_file)) {
        return false;
    }

    $module_status = json_decode((string) file_get_contents($status_file));
    return is_object($module_status) && property_exists($module_status,$name) ? (bool) $module_status->$name : false;
}
